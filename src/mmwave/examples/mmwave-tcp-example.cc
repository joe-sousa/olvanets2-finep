...
main (int argc, char *argv[])
{
	...
	//Config::SetDefault ("ns3::MmWaveHelper::HarqEnabled", BooleanValue(harqEnabled));    //enable hybrid ARQ
	...
	enbPositionAlloc->Add (Vector (40.0, 0.0, 3.0));
	...
	uePositionAlloc->Add (Vector (800.0, 0.0, 0.0));
	...
	/*MobilityHelper dgwmobility;
	Ptr<ListPositionAllocator> dgwPositionAlloc = CreateObject<ListPositionAllocator> ();
	dgwPositionAlloc->Add (Vector (40.0, 40.0, 0.0));
	dgwmobility.SetMobilityModel ("ns3::ConstantPositionMobilityModel");
	dgwmobility.SetPositionAllocator(dgwPositionAlloc);
	dgwmobility.Install (ueNode);*/
	...
	app->Setup (ns3TcpSocket, sinkAddress, 100000, 1, DataRate ("1000Mb/s"));
	...
	AnimationInterface anim ("mmwave-tcp-example.xml");
